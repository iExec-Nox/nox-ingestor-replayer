...
> [!IMPORTANT]
> Because a replayed message is indistinguishable from a live one, `blockNumber` **is not monotonic on the stream**: replaying an old range puts low block numbers after higher ones that are already stored. Do not implement a `blockNumber` high-watermark that drops anything below the last-seen value — it will silently discard every replayed message. The replay reports success on `/metrics` and changes nothing downstream.
